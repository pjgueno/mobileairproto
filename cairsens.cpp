...
CairsensUART::EError CairsensUART::getInstantVal(uint8_t& arg_u8_instVal)
{
  ...
  {
    Debug.println("Timeout when getting GetInstantValue RESP");
    return TIMEOUT;
  }
  ...
  {
    Debug.println("Bad field in response - try to empty stream");
  ...
  {
    Debug.println("Bad CRC - try to empty stream");
    _p_uartStream->readBytes(loc_au8_getInstValFrame, sizeof(loc_au8_getInstValFrame)); 
    return  BAD_CRC; 
  }
  /** TODO check Life */
